            ...
            for root_dir, _, files in os.walk(directory_path):
                for file in files:
                    if file.lower().endswith('.pdf'):
                        process_pdf(os.path.join(root_dir, file))
...
# Create a button to select the PDF files or directory
select_button = tk.Button(root, text="Chọn file PDF hoặc thư mục", command=select_files_or_directory)
select_button.pack(pady=10)

# Create a button to select a directory
select_directory_button = tk.Button(root, text="Chọn thư mục", command=lambda: directory())
